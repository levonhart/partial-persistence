...
int print_tree(persytree_t * tree, node_t * root, unsigned version, FILE * out, unsigned depth) {
	...
	if (left_child != NULL) {
		retl = print_tree(tree, left_child, version, out, depth + 1);
		retl++;
		fprintf(out, " ");
	}
	retp = fprintf(out, "%d,%u,%c", node_get(tree, root, key, version, int), depth,
	               node_get(tree, root, color, version, char) == 'r' ? 'R' : 'N');
	if (right_child != NULL) {
		fprintf(out, " ");
		retr = print_tree(tree, right_child, version, out, depth + 1);
		retr++;
	}
...
void parse_file(FILE * file, persytree_t * dest, FILE * out) {
	...
	while (fscanf(file, "\t%" CSZ "s", command) != EOF) {
		if (strncmp(command, "INC", COMMAND_SIZE) == 0) {
			...
			//        (int)dest->root[dest->last_version]->left, (int)dest->root[dest->last_version]->right);
			// printf(" mods:{ \n");
			// for (int j = 0;
			//      dest->root[dest->last_version] != NULL && j < dest->root[dest->last_version]->n_mods; j++) {
